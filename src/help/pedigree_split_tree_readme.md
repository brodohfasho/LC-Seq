...
> Did chromatograms for members of this class show a statistically significant peak **after** the parent’s retention time (plus **null RT threshold**)?

The same logic powers **lineage analysis** for a single compound; pedigree runs it **library-wide** via **Library Analysis → RT assignment** (Pedigree mode).
...
## Tree display controls (Pedigree visualization tab)
...
The **tier summary** cards and **Export pedigree CSV** always include all evaluated tiers; display filters affect only the figure.

## How the image is generated (technical)

1. Load compounds and build a chromatogram map (count channel, time unit, peak picker settings).
...
- **Product peak prominence** is in **`product_prominence.csv`** inside **Export analysis bundle** when pedigree RT assignment was run — compare with bulk Library QC S/N, which uses the tallest significant peak and may not be the product.

## Related help

Use **Help ▾** on the RT assignment tab for pedigree settings and the **Export analysis bundle glossary**.
